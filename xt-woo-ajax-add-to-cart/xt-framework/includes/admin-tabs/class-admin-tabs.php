...
if(!class_exists('XT_Framework_Admin_Tabs')) {
    ...
    abstract class XT_Framework_Admin_Tabs {
        ...
        public function enqueue_scripts() {
            ...
            wp_add_inline_script( 'xt-follow', 'XT_FOLLOW.init();' );
        ...
        public function show_tab( $page_content = null ) {
            ...
            if ( ! empty( $page_content ) ) {
                ...
                // Freemius supplies a complete, internally generated account/checkout template.
                // It contains forms, inputs, and SDK scripts that wp_kses_post() intentionally
                // removes, so keep the trusted SDK response distinct from ordinary tab HTML.
                $tab['content']['trusted_sdk_html'] = true;
            }
            ...
            if ( ! empty( $tab['content'] ) ) {
                ...
                if ( $content_type === 'template' ) {
                    ...
                    if ( ! empty( $content['function'] ) ) {
                        $callback_output = call_user_func_array( $content['function'], $args );
                        if ( is_string( $callback_output ) ) {
                            echo wp_kses_post( $callback_output );
                        }
                    }

                } else if ( $content_type === 'html' ) {

                    if ( ! empty( $content['trusted_sdk_html'] ) ) {
                        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Trusted Freemius SDK template generated by the bundled SDK, not user input.
                        echo $content['html'];
                    } else {
                    ...
                    $remote_content = $this->remote_get( $content['url'], $json_decode );
                    if ( is_string( $remote_content ) ) {
                        echo wp_kses_post( $remote_content );
                    }
        ...
        public function remote_get( $url, $json_decode = false ) {
            ...
            // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only remote cache bypass.
            $bypass_cache = isset( $_GET['nocache'] ) && 1 === absint( wp_unslash( $_GET['nocache'] ) );

            if ( false === $content || $bypass_cache ) {
